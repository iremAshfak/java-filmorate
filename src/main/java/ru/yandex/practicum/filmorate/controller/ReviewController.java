...
public class ReviewController {
    ...
    @GetMapping
    public List<ReviewDTO> getReviewsOfFilm(@RequestParam(required = false) Optional<Long> filmId, @RequestParam(defaultValue = "10") Integer count) {
        if (filmId.isPresent()) {
    ...
    @ResponseStatus(HttpStatus.OK)
    @GetMapping("/{id}")
    public ReviewDTO getReviewsById(@PathVariable("id") Long reviewId) {
        return reviewService.getById(reviewId);
    }

    @ResponseStatus(HttpStatus.CREATED)
    @PostMapping
    public ReviewDTO create(@RequestBody @Valid CreateReviewRequest request) {
        return reviewService.create(request);
    }

    @ResponseStatus(HttpStatus.OK)
    @PutMapping
    public ReviewDTO update(@RequestBody @Valid UpdateReviewRequest request) {
